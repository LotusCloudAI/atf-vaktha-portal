...
export const processSpeechStorage = onObjectFinalized(async (event: StorageEvent) => {
  try {
    ...
    // 🔥 FINAL FIX — CORRECT DOC ID EXTRACTION
    const docId = fileName.replace(ext, "");

    if (!SUPPORTED_AUDIO.includes(ext)) return;
    ...
    logger.log("Saved AI data:", docId);

    if (userId) {
      await db.collection("users").doc(userId).update({
        points: admin.firestore.FieldValue.increment(10),
      });
      logger.log("Points added:", userId);
    }
