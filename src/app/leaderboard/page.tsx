...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      try {
        const q = query(
          collection(db, "speeches"), // ✅ FIXED
          orderBy("speechScore", "desc"), // ✅ FIXED
          limit(10)
        );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="text-center mb-12">
          ...
          <p className="text-slate-500 mt-2 font-medium">
            Top performing speakers across the platform.
          </p>
        </header>
        ...
        <div className="space-y-4">
          {speakers.map((speaker, index) => (
            <div
              ...
              className={`p-6 rounded-3xl border flex items-center justify-between transition-all ${
                index === 0
                  ? "bg-yellow-50 border-yellow-200 scale-105 shadow-xl"
              ...
              <div className="flex items-center gap-6">
                <span
                  className={`text-2xl font-black ${
                    index === 0 ? "text-yellow-600" : "text-slate-300"
                  }`}
                >
                  #{index + 1}
                </span>
                ...
                <div>
                  <p className="font-bold text-slate-900 text-lg">
                    Member {speaker.userUid?.slice(0, 6) || "User"}
                  </p>
          ...
          {loading && (
            <p className="text-center py-20 animate-pulse text-slate-400 font-bold uppercase tracking-widest">
